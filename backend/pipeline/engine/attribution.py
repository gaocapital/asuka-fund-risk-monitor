"""
pipeline.engine.attribution — Source attribution audit for position data.
==========================
...
SNAPSHOT_DIR = Path("attribution_snapshots")  # repo-root relative
...
def classify_price(p: dict) -> tuple[str, int, str]:
    ...
    # Bloomberg / IB Gateway via canonical names OR text descriptions
    if "bloomberg" in src_lower:
        return ("VERIFIED", 0, src[:60])
    if "ib " in src_lower or "ib_gateway" in src_lower or "interactive brokers" in src_lower:
        return ("VERIFIED", 0, src[:60])
    if "csv" in src_lower or "manual" in src_lower or "pm refresh" in src_lower or "pm input" in src_lower:
        return ("MANUAL", 1, src[:60])
    if src == "yahoo" or src == "yahoo_eod":
        return ("VERIFIED", 0, "yahoo_eod (delayed close)")
    if not src:
        return ("UNKNOWN", 4, "no price_source field")
    return ("UNKNOWN", 3, f"unrecognized source: {src[:50]}")
