...
def ingest_tdnet(data_path: str, tdnet_path: str, apply_tilts: bool = True) -> dict:
    ...
    # A successful real TDNet scan supersedes any prior backfill — clear the
    # tdnet_backfilled_from_filings flag on every position. Without this, the
    # flag set during a one-off schema migration would stick forever and the
    # attribution-health pill would stay red even after fresh scans land.
    n_cleared = 0
    for p in data.get("positions", []):
        if p.pop("tdnet_backfilled_from_filings", None):
            n_cleared += 1
    if n_cleared > 0:
        print(f"  ✓ Cleared stale tdnet_backfilled_from_filings flag on {n_cleared} positions (real scan supersedes backfill)")
