...
|---|---|---|---|---|
| 1 | Price refresh (chain: Yahoo intraday → IB → BBG → Yahoo EOD) | `yahoo_intraday_price_pull.py` etc. | ✓ | 5-30s |
...
| Failure | Result |
|---|---|
| Yahoo intraday API rate-limited (HTTP 429) | Module retries with exponential backoff; falls through to IB → Bloomberg → Yahoo EOD |
| IB Gateway down | Falls through to Bloomberg → Yahoo EOD |
| Bloomberg + IB both down | Stays on Yahoo intraday (free, no auth); falls back to Yahoo EOD if intraday endpoint blocked |
| All price sources fail | Action engine emits STALE_INPUTS based on `price_date` age |
...
```cmd
python run_daily_dashboard.py --price-source yahoo_intraday   :: default — free, no auth
python run_daily_dashboard.py --price-source ib               :: real-time TSE via IB Gateway
python run_daily_dashboard.py --price-source bbg              :: Bloomberg PX_LAST
python run_daily_dashboard.py --price-source yahoo            :: Yahoo EOD (legacy fallback)
```

### Yahoo intraday chip (default source)

Each position's freshness chip is now decorated with market state + day-over-day change:
...
Hover for tooltip: `14:30 JST · today · src: yahoo_intraday · market: REGULAR`

Yahoo TSE feed has a **20-minute delay** for free users — fine for daily refresh,
not appropriate for tight-tolerance L4 ARB or breaking news. Use IB or BBG for those.
