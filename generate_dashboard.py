...
_MEMO_MANIFEST_CACHE: dict | None = None


def load_memo_manifest() -> dict:
    """Load memos/manifest.json once and cache. Returns {ticker: meta_dict}."""
    global _MEMO_MANIFEST_CACHE
    if _MEMO_MANIFEST_CACHE is not None:
        return _MEMO_MANIFEST_CACHE
    path = Path("memos") / "manifest.json"
    if not path.exists():
        _MEMO_MANIFEST_CACHE = {}
        return _MEMO_MANIFEST_CACHE
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        _MEMO_MANIFEST_CACHE = data.get("memos", {})
    except (json.JSONDecodeError, OSError):
        _MEMO_MANIFEST_CACHE = {}
    return _MEMO_MANIFEST_CACHE


def render_memo_chip(ticker: str) -> str:
    """Return a small memo chip for the position row — green/amber/red by age,
    🔒 if external (analyst-pasted from Asuka Event Driven Project), with a
    link to the .md file. No chip if no memo exists for this ticker."""
    manifest = load_memo_manifest()
    ...
    age_h = 999
    try:
        gen_dt = datetime.fromisoformat(gen.split("+")[0])
        age_h = (datetime.now() - gen_dt).total_seconds() / 3600
    except (ValueError, TypeError):
        pass
    if src == "external" or m.get("do_not_auto_refresh"):
        icon = "🔒"
        color_class = "memo-external"
        title = f"External memo (AEDP) · do_not_auto_refresh · pasted {gen[:10]}"
    elif age_h < 24:
        icon = "📝"
        color_class = "memo-fresh"
        title = f"Auto memo · refreshed {age_h:.1f}h ago"
    elif age_h < 72:
        icon = "📝"
        color_class = "memo-stale"
        title = f"Auto memo · {age_h/24:.1f}d old"
    else:
        icon = "📝"
        color_class = "memo-very-stale"
        title = f"Auto memo · {age_h/24:.0f}d old — needs refresh"
    summary = (m.get("summary_line") or "")[:200]
    if summary:
        title = title + " · " + summary
    # Surface AEDP playbook stage if available
    stage = m.get("anchor_playbook_stage", "")
    if stage:
        title = title + " · " + stage[:80]
    n_chain = m.get("n_anchor_chain_filings", 0)
    if n_chain:
        title = title + f" · chain={n_chain}"
    # Link to the .md file via file:// (works on Windows in default editor)
    href = f"memos/{ticker}.md"
    return (
        f'<a class="memo-chip {color_class}" href="{href}" '
        f'title="{html_escape(title)}" target="_blank">{icon}</a>'
    )
...
CSS_STYLE = """
:root {
  --bg: #0A0D12; --surface: #141923; --surface-2: #1C2230; --surface-3: #252C3B;
  --border: #232A38; --border-strong: #333C4D;
  --text: #ECEAE4; --text-muted: #939BAA; --text-dim: #616B7A;
  ...
  --shadow: 0 1px 2px rgba(0,0,0,0.45), 0 12px 32px -10px rgba(0,0,0,0.6);
...
body { padding: 30px 52px 90px; max-width: 1860px; margin: 0 auto; }
...
.kpi-row { display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; margin-top: 28px; }
@media (max-width: 900px) { .kpi-row { grid-template-columns: repeat(2, 1fr); } }
.kpi { background: var(--surface); padding: 18px 20px; border: 1px solid var(--border);
...
.kpi-row-secondary { grid-template-columns: repeat(5, 1fr); margin-top: 12px; }
...
.priority { background: var(--surface); border: 1px solid var(--border-strong); border-left: 3px solid var(--accent);
  padding: 24px 30px; margin-bottom: 28px; border-radius: var(--radius); box-shadow: var(--shadow); }
...
.filings { background: var(--surface); border: 1px solid var(--border-strong); border-left: 3px solid var(--gold);
  padding: 24px 30px; margin-bottom: 36px; border-radius: var(--radius); box-shadow: var(--shadow); }
...
/* Per-position shadow-buy memo chips */
.memo-chip {
  display: inline-block;
  margin-left: 6px;
  padding: 1px 5px;
...
.memo-chip:hover { transform: translateY(-1px); transition: transform 0.1s; }
.memo-chip.memo-external    { background: rgba(34, 197, 94, 0.22); border-color: #22C55E; }
.memo-chip.memo-fresh       { background: rgba(34, 197, 94, 0.10); border-color: rgba(34, 197, 94, 0.4); }
.memo-chip.memo-stale       { background: rgba(245, 158, 11, 0.10); border-color: rgba(245, 158, 11, 0.4); }
.memo-chip.memo-very-stale  { background: rgba(239, 68, 68, 0.10);  border-color: rgba(239, 68, 68, 0.4); }
...
.panel { background: var(--surface); border: 1px solid var(--border-strong); padding: 24px 28px; border-radius: var(--radius); box-shadow: var(--shadow); }
...
.thesis-card { background: var(--surface); border: 1px solid var(--border-strong); border-left: 2px solid var(--border-strong); padding: 20px 22px; border-radius: var(--radius); box-shadow: var(--shadow-sm); }
...
def render_freshness_banner(data: dict, positions: list) -> str:
    ...
    # Counts positions whose key fields are well-attributed. Mirrors the
    # semantics of source_attribution_audit.py: VERIFIED is the gold standard
    # but PROXY (e.g. structured last_filing without an edinet_code, or
    # MANUAL = PM-typed) still counts as "we know where this came from."
    # ESTIMATED and UNKNOWN do NOT count — those are real provenance gaps.
    n_pos = max(len(positions), 1)
    fully_verified = 0
    for p in positions:
        # (1) price provenance: yahoo_intraday/ib/bloomberg/manual_csv all OK
        price_src = (p.get("price_source") or "").lower()
        price_ok = price_src in ("yahoo_intraday", "ib_gateway", "bloomberg", "manual_csv")

        # (2) Asuka WAC provenance: anything not "estimated" and not empty
        wac_src = (p.get("wac_source") or "").lower()
        wac_ok = bool(wac_src) and "estimat" not in wac_src

        # (3) last_filing record: structured dict with at least a date and filer
        # (edinet_code is nice-to-have but not required — many manual desk-note
        # records lack it because the upstream filing_parser isn't wired in)
        lf = p.get("last_filing") or {}
        if not isinstance(lf, dict):
            lf = {}
        filings_ok = bool(lf.get("date") and lf.get("filer"))

        # (4) TDNet: scan ran (date stamped) and not relying on legacy backfill.
        # tdnet_scan.py now clears tdnet_backfilled_from_filings on every run.
...
def render_position_row(p: dict, delta: dict) -> str:
    ...
    return f"""
        <tr data-ticker="{p['ticker']}" class="{row_class}">
          ...
          {mos_cell_html}
          {asset_mos_cell_html}
          <td><span class="action {action_css}"{action_title}>{action_label}</span>{tier_badge_html}{action_change_html} {render_memo_chip(p['ticker'])}</td>
...
def generate(data_path: str, output_path: str, state_dir: str = STATE_DIR) -> dict:
    ...
    # Recompute PWER + activist_pwer from CURRENT spot price before metrics.
    # Static stored pwer is preserved as p["pwer_stored"] for drift detection.
    # This makes every dashboard open reflect the latest spot, not the value
    # that was calibrated at a stale price weeks ago.
    n_recomputed = 0
